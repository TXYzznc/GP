...
- ⚠️ **Tab数量规则（奇数原则）**：任意两个相邻非空元素(主要是指有效数据,而不是表头元素)之间的Tab数量**必须是奇数**（1、3、5…）
  - 原理：
    - 相邻两个非空列之间：`A \t B` = **1个Tab**（奇数 ✓）
    - 中间有1个空列：`A \t [空列本身也是\t] \t B` = `A \t\t\t B` = **3个Tab**（奇数 ✓）
    - 中间有2个空列：**5个Tab**（奇数 ✓）
    - 规律：N个空列 → (2N+1) 个Tab，永远是奇数
  - 验证方法：生成TXT后扫描，任意两个非空值之间数Tab数，必须是奇数；偶数说明少写或多写了一个Tab
  - 常见错误：`1001\t\t神圣庇护`（2个Tab=偶数），中间有1个空备注列时正确写法是 `1001\t\t\t神圣庇护`（3个Tab）
- ⚠️ **数组分隔符规则**：
  - **数组元素必须使用英文逗号 `,` 分隔**
  - **禁止使用中文顿号 `、` 或其他分隔符**

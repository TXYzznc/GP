...
def find_available_unity_port(start_port: int = PORT_RANGE_START, end_port: int = PORT_RANGE_END, timeout: float = 0.5) -> Optional[int]:
    """
    在指定端口范围内查找可用的 Unity 服务器端口。
    优先检查注册表中记录的端口，如果不可用则轮询端口范围。
    
    Args:
        start_port: 起始端口（默认 8090）
        end_port: 结束端口（默认 8100）
        timeout: 每个端口的连接超时时间（秒）
    
    Returns:
        可用的端口号，如果没有找到则返回 None
    """
    # 1. 优先检查注册表中的端口
    ...
    def __init__(self, port: int = None, target: str = None, url: str = None):
        ...
        if not self.url:
            if port:
                ...
                # 默认情况下，尝试轮询查找可用端口
                found_port = find_available_unity_port()
                if found_port:
                    self.url = f"http://127.0.0.1:{found_port}"
                else:
                    # 如果没有找到可用端口，使用默认端口
...
def wait_for_unity(timeout: float = 10.0, check_interval: float = 1.0) -> bool:
    """
    等待 Unity REST 服务器变为可用状态。在脚本创建后很有用。
    会在端口范围内轮询查找可用的 Unity 实例。
    """
    start_time = time.time()
    while time.time() - start_time < timeout:
        # 尝试查找可用端口
        found_port = find_available_unity_port(timeout=0.5)
        if found_port:
            # 更新全局默认客户端的 URL
            global _default_client
            _default_client.url = f"http://127.0.0.1:{found_port}"
            return True

...
public static class EntityExtension
{
    ...
    /// <returns></returns>
    public static int ShowEffect(this EntityComponent eCom, int effectId, EntityParams eParams, float lifeTime = 3, int? sortLayer = null)
    {
        var onShow = eParams.OnShowCallback;
        eParams.OnShowCallback = entity =>
        {
            onShow?.Invoke(entity);
            if (sortLayer != null)
            {
                var renderers = entity.GetComponentsInChildren<Renderer>();
                foreach (var renderer in renderers)
                {
                    renderer.sortingOrder = sortLayer.Value;
                }
            }
            if (lifeTime > 0)
            {
        ...
        string assetName = ResourceExtension.GetResourceConfigPath(effectId);
        return eCom.ShowEntity<EntityBase>(assetName, Const.EntityGroup.Effect, eParams);
    }

    public static void ShowPopEmoji(this EntityComponent eCom, int emojiId, EntityParams eParams, Vector3 endPos, float duration = 2)
    {
        ...
        string assetName = ResourceExtension.GetResourceConfigPath(emojiId);
        eCom.ShowEntity<EntityBase>(assetName, Const.EntityGroup.Effect, eParams);
    }
    /// <summary>
    /// 在UI屏幕空间创建飘字
    /// </summary>
    ...
    public static void PopScreenText(this EntityComponent eCom, string text, Vector3 startWorldPos, float popDistance, float duration = 1f, float fontSize = 4f)
    {
        ...
        // TODO: 请在 ResourceConfigTable 中配置 "Effect/PopText" 并在此处替换为对应的 ID
        // int popTextId = Const.Effect.PopText; 
        // string assetName = ResourceExtension.GetResourceConfigPath(popTextId);
        // eCom.ShowEntity<EntityBase>(assetName, Const.EntityGroup.Effect, effectParms);
        
        // 临时兼容代码：
        Log.Warning("PopScreenText: 尚未配置 Effect/PopText 的资源ID，请尽快修改。");
        // eCom.ShowEntity<EntityBase>("Effect/PopText", Const.EntityGroup.Effect, effectParms);
    }
    /// <summary>
    /// 创建飘字效果
    /// </summary>
    ...
    public static void ShowPopText(this EntityComponent eCom, EntityParams eParams, string content, Vector3 endPos, float duration = 0.5f, float fontSize = 5f)
    {
        ...
        eCom.ShowEntity<EntityBase>("Effect/MoneyText", Const.EntityGroup.Effect, eParams);
    }
    /// <summary>
    /// 创建Entity
    /// </summary>

...
public static class CardEffectHelper
{
    ...
    public static void DealDamage(ChessEntity target, float damage, int damageType = 0, ChessAttribute attacker = null)
    {
        ...
        {
            DebugEx.Error("CardEffectHelper", "目标或目标属性为空");
            return;
        }
        ...
        DebugEx.Log("CardEffectHelper", $"对 {target.Config?.Name} 造成 {damage} 伤害 (类型: {damageType})");
    }
    ...
    public static void HealTarget(ChessEntity target, float healAmount)
    {
        ...
        DebugEx.Log("CardEffectHelper", $"恢复 {target.Config?.Name} {healAmount} HP");
    }
    ...
    public static void ApplyBuff(ChessEntity target, int buffId, GameObject caster = null)
    {
        if (target == null)
        {
            DebugEx.Error("CardEffectHelper", "目标为空");
            return;
        }
        ...
        if (buffManager == null)
        {
            DebugEx.Error("CardEffectHelper", $"{target.Config?.Name} 没有 BuffManager 组件");
            return;
        }

        buffManager.AddBuff(buffId, caster);
        DebugEx.Log("CardEffectHelper", $"对 {target.Config?.Name} 应用 Buff: {buffId}");
    }
    ...
    public static void PlayEffect(int effectId, Vector3 position, ChessEntity target = null)
    {
        if (effectId <= 0)
        {
            DebugEx.Warning("CardEffectHelper", "特效 ID 无效");
            return;
        }

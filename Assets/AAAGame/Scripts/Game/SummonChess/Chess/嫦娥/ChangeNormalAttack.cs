...
public class ChangeNormalAttack : ChessNormalAttackBase
{
    ...
    public override void ExecuteAttack(ChessEntity caster, ChessEntity target)
    {
        ...
        // 5. 启动命中检测（投射物会飞向目标，命中时伤害才会计算）
        DebugEx.Log(nameof(ChangeNormalAttack), "[进入检测] 启动投射物检测，伤害延迟到命中时");
        ...
        DebugEx.Success(nameof(ChangeNormalAttack), "普攻启动完成，投射物已发射（伤害计算延迟到命中时刻）");
    }

...
public class ChangeUltimate : ChessSkillBase
{
    ...
    public override void Init(ChessContext ctx, SummonChessSkillTable config)
    {
        ...
        DebugEx.Log(nameof(ChangeUltimate), "月华天倾初始化完成");
    }

    public override bool TryCast()
    {
        DebugEx.Log(nameof(ChangeUltimate), "→ 尝试释放大招「月华天倾」");

        if (!base.TryCast())
        {
            DebugEx.Warning(nameof(ChangeUltimate), "  ✗ 大招释放条件不满足（MP不足或冷却中）");
            return false;
        }
    ...
    {
        DebugEx.Log(nameof(ChangeUltimate), "→ ExecuteSkill: 开始执行大招");

        if (caster == null)
        {
            DebugEx.Error(nameof(ChangeUltimate), "  ✗ 施法者为 null，无法执行技能");
            return;
        }

        DebugEx.Log(nameof(ChangeUltimate), $"  ├─ 施法者: {caster.Config?.Name}");

        // 播放技能释放特效（施法者位置）
        DebugEx.Log(nameof(ChangeUltimate), "  ├─ 播放技能释放特效...");
        PlaySkillEffect(caster);
        DebugEx.Log(nameof(ChangeUltimate), "  ├─ ✓ 特效播放完成");

        // 异步创建法阵
        DebugEx.Log(nameof(ChangeUltimate), "  └─ 启动异步创建法阵流程...");
    ...
    {
        DebugEx.Log(nameof(ChangeUltimate), "→ 开始异步创建法阵...");

        // 获取目标敌人
        ChessEntity targetEnemy = FindNearestEnemy(caster);
        if (targetEnemy == null)
        {
            DebugEx.Warning(nameof(ChangeUltimate), "  ├─ ✗ 未找到目标敌人");
            return;
        }

        Vector3 targetPosition = targetEnemy.transform.position;
        DebugEx.Log(nameof(ChangeUltimate), $"  ├─ 目标敌人: {targetEnemy.Config?.Name}, 位置: {targetPosition}");

        // 从 CustomData 读取法阵预制体配置
        var customData = ParseCustomData(m_Config.CustomData);
        if (customData == null || customData.MagicCircleId <= 0)
        {
            DebugEx.Error(nameof(ChangeUltimate), "  ├─ ✗ CustomData 中未找到有效的 MagicCircleId");
            return;
        }
        ...
        DebugEx.Log(nameof(ChangeUltimate), $"  ├─ CustomData 解析成功: PrefabId={magicCirclePrefabId}, ProjectileSpawnHeight={projectileSpawnHeight}");

        // 异步加载法阵预制体
        DebugEx.Log(nameof(ChangeUltimate), $"  ├─ 正在加载法阵预制体 (ID={magicCirclePrefabId})...");
        GameObject prefab = await ResourceExtension.LoadPrefabAsync(magicCirclePrefabId);
        if (prefab == null)
        {
            DebugEx.Error(nameof(ChangeUltimate), $"  ├─ ✗ 法阵预制体加载失败 (ID={magicCirclePrefabId})");
            return;
        }
        DebugEx.Log(nameof(ChangeUltimate), $"  ├─ ✓ 法阵预制体加载成功");
        ...
        // 实例化法阵
        DebugEx.Log(nameof(ChangeUltimate), $"  ├─ 实例化法阵预制体...");
        ...
        DebugEx.Log(nameof(ChangeUltimate), $"  ├─ ✓ 法阵实例化成功，位置={spawnPosition}");

        // 获取并初始化 ChangeMagicCircle 组件
        var magicCircle = circleInstance.GetComponent<ChangeMagicCircle>();
        if (magicCircle == null)
        {
            DebugEx.Error(nameof(ChangeUltimate), "  ├─ ✗ 法阵预制体上未找到 ChangeMagicCircle 组件");
            Object.Destroy(circleInstance);
            return;
        }

        DebugEx.Log(nameof(ChangeUltimate), "  ├─ 初始化 ChangeMagicCircle 组件...");
    ...
    private CustomDataWrapper ParseCustomData(string customData)
    {
        ...
        {
            DebugEx.Warning(nameof(ChangeUltimate), "CustomData 为空");
            return null;
        }
        ...
        {
            DebugEx.Error(nameof(ChangeUltimate), $"✗ 解析 CustomData 失败: {customData}\n原因: {ex.Message}");
            return null;
        }
    }

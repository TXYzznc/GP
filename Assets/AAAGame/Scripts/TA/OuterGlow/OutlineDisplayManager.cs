...
/// </summary>
public class OutlineDisplayManager : SingletonBase<OutlineDisplayManager>
{
    #region 配置
    [System.Serializable]
    ...
    [SerializeField]
    private int _activeOutlines;

    [SerializeField]
    private int _layerChanges;

    [SerializeField]
    private float _scanTime;

    [SerializeField]
    private float _layerCheckTime;
    ...
    #region 初始化
    /// <summary>
    /// 异步初始化 - 等待摄像机加载完成后再启动系统
    /// </summary>
    private IEnumerator InitializeAsync()
    {
        DebugEx.LogModule("OutlineManager", "开始异步初始化...");

        float timeout = 5f;
        float elapsed = 0f;

        while (TargetCamera == null && elapsed < timeout)
        {
            // ⭐ 通过 CameraRegistry 获取玩家摄像机
            TargetCamera = CameraRegistry.PlayerCamera;
        ...
        if (TargetCamera != null)
        {
            DebugEx.LogModule("OutlineManager", $"✅ 成功找到摄像机: {TargetCamera.name}");
        }
        else
        {
            DebugEx.WarningModule(
                "OutlineManager",
                $"⚠️ 等待{timeout}秒后仍未找到摄像机，使用CameraRegistry备用"
            );
            TargetCamera = CameraRegistry.PlayerCamera;
        }
        ...
        if (TargetCamera == null)
        {
            DebugEx.ErrorModule("OutlineManager", "❌ 无法找到任何可用的摄像机！系统将无法正常工作");
            _debugInfo = "错误：无摄像机";
            yield break;
        }
        ...
        if (OutlineRenderFeature.Instance == null)
        {
            DebugEx.ErrorModule(
                "OutlineManager",
                "OutlineRenderFeature.Instance 为空！请确认已在URP Renderer中添加OutlineRenderFeature"
            );
            _debugInfo = "错误：OutlineRenderFeature未配置";
        }
        else
        {
            DebugEx.LogModule("OutlineManager", "✅ 初始化完成 - 统一管控模式");
            _debugInfo = "运行正常";
        }
        ...
        DebugEx.LogModule("OutlineManager", "所有系统已启动");
    }
    ...
    private void BuildLayerConfigMap()
    {
        ...
        foreach (var mapping in LayerMappings)
        {
            ...
            if (layerIndex < 0)
            {
                DebugEx.ErrorModule(
                    "OutlineManager",
                    $"Layer '{mapping.LayerName}' 不存在！请检查Layer名称拼写"
                );
                continue;
            }
            ...
            DebugEx.LogModule(
                "OutlineManager",
                $"✅ 映射成功: Layer[{layerIndex}] '{mapping.LayerName}' -> '{mapping.OutlineConfig.name}'"
            );
        }

        if (_layerConfigMap.Count == 0)
        {
            DebugEx.WarningModule("OutlineManager", "警告：没有配置任何Layer映射！");
    ...
    private void HandleLayerChange(TrackedObject tracked, int newLayer)
    {
        ...
        DebugEx.LogModule(
            "OutlineManager",
            $"⭐ Layer变化: {tracked.GameObject.name} [{oldLayerName}] -> [{newLayerName}]"
        );
        ...
        if (_layerConfigMap.TryGetValue(newLayer, out OutlineConfig newConfig))
        {
            ...
            {
                _highPriority.Enqueue(tracked);
                DebugEx.LogModule("OutlineManager", $"✅ 应用新配置: {newConfig.name}");
            }
        }
        else
        {
            // ✅ 新Layer无配置，只是隐藏外轮廓，但保留组件
            tracked.CurrentConfig = null;
            DebugEx.LogModule("OutlineManager", "新Layer无配置，外轮廓已隐藏（组件保留）");
        }
    }
    ...
    void OnDrawGizmosSelected()
    {
        ...
        if (_tracked != null)
        {
            foreach (var tracked in _tracked.Values)
            {
                ...
                Gizmos.color =
                    tracked.Component != null && tracked.Component.IsOutlineActive
                        ? Color.green
                        : Color.red;

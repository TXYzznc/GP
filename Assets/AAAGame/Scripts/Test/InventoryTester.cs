...
public class InventoryTester : MonoBehaviour
{
    ...
    private void Awake()
    {
        DebugEx.Log(nameof(InventoryTester), "背包测试器初始化");
    ...
    {
        DebugEx.Log(nameof(InventoryTester), "开始加载物品列表");

        if (ItemManager.Instance == null)
        {
            DebugEx.Error(nameof(InventoryTester), "ItemManager 创建失败，无法加载物品列表");
            return;
        }
        ...
        if (table == null)
        {
            DebugEx.Error(nameof(InventoryTester), "ItemTable 配置表未加载");
            return;
        }
        ...
        {
            DebugEx.Error(nameof(InventoryTester), "ItemTable 配置表为空");
            return;
        }

        DebugEx.Log(nameof(InventoryTester), $"获取到 {allRows.Length} 条物品数据");
        ...
        foreach (var row in allRows)
        {
            ...
            if (itemData == null)
            {
                DebugEx.Warning(nameof(InventoryTester), $"物品ID {row.Id} 数据为空");
                continue;
            }
    ...
    public void OpenInventoryUI()
    {
        ...
        DebugEx.Success(nameof(InventoryTester), "背包UI已打开");
    }
    ...
    public void CloseInventoryUI()
    {
        ...
        {
            DebugEx.Error(nameof(InventoryTester), "无法获取背包UI资源名称");
            return;
        }
        ...
        if (inventoryUI != null)
        {
            ...
            DebugEx.Success(nameof(InventoryTester), "背包UI已关闭");
        }
        else
        {
            DebugEx.Warning(nameof(InventoryTester), "背包UI未打开");
        }
    }
    ...
    public void TestAddItems()
    {
        ...
        {
            DebugEx.Error(nameof(InventoryTester), "InventoryManager 未初始化");
            return;
        }
        ...
        {
            DebugEx.Warning(nameof(InventoryTester), "物品列表为空，尝试再次加载配置表");
            ...
            if (m_StackableItemIds.Count == 0 && m_NonStackableItemIds.Count == 0)
            {
                DebugEx.Error(nameof(InventoryTester), "物品列表为空，请检查配置表是否已加载");
                DebugEx.Error(nameof(InventoryTester), "可能的原因：");
                DebugEx.Error(nameof(InventoryTester), "1. ItemTable 配置表数据为空");
                DebugEx.Error(nameof(InventoryTester), "2. ItemManager.LoadItemTable() 加载失败");
                DebugEx.Error(nameof(InventoryTester), "3. 所有物品的 ItemData 都为 null");
                return;
            }
        }
        ...
        {
            DebugEx.Warning(nameof(InventoryTester), "没有可用物品来添加");
            return;
        }
        ...
        DebugEx.Success(nameof(InventoryTester), $"添加物品测试完成 (共 {selectedItems.Count} 种物品)");
    }
    ...
    public void TestRemoveItems()
    {
        ...
        DebugEx.Success(nameof(InventoryTester), "移除物品测试完成");
    }
    ...
    public void TestUseItem()
    {
        ...
        for (int i = 0; i < slots.Count; i++)
        {
            if (!slots[i].IsEmpty)
            {
                ...
                DebugEx.Log(nameof(InventoryTester), $"尝试使用格子 {i} 的物品: {item.Name}");
        ...
        DebugEx.Success(nameof(InventoryTester), "使用物品测试完成");
    }
    ...
    public void TestStackableItems()
    {
        ...
        if (m_StackableItemIds.Count == 0)
        {
            DebugEx.Warning(nameof(InventoryTester), "没有可堆叠物品");
            return;
        }
        ...
        // 多次添加同一物品，测试堆叠
        DebugEx.Log(nameof(InventoryTester), $"连续添加物品 {itemName}，测试堆叠功能");
        ...
        DebugEx.Success(nameof(InventoryTester), "可堆叠物品测试完成");
    }
    ...
    public void TestFullInventory()
    {
        ...
        {
            DebugEx.Warning(nameof(InventoryTester), "没有不可堆叠物品");
            return;
        }
        ...
        DebugEx.Log(nameof(InventoryTester), $"当前背包状态: {usedSlots}/{maxSlots} (已用/总数)");

        // 尝试填满背包
        DebugEx.Log(nameof(InventoryTester), "尝试填满背包...");
        int addCount = 0;
        for (int i = usedSlots; i < maxSlots + 5; i++)
        {
            ...
            {
                DebugEx.Warning(nameof(InventoryTester), $"背包已满，成功添加 {addCount} 个物品");
                break;
            }
        }

        DebugEx.Success(nameof(InventoryTester), "背包满测试完成");
    }
    ...
    public void TestSaveAndLoad()
    {
        ...
        // 保存当前背包数据
        DebugEx.Log(nameof(InventoryTester), "保存背包数据...");
        var saveData = InventoryManager.Instance.SaveInventory();
        DebugEx.Log(nameof(InventoryTester), $"保存了 {saveData.Count} 个物品");

        // 添加一些新物品
        if (m_StackableItemIds.Count > 0)
        {
            DebugEx.Log(nameof(InventoryTester), "添加新物品以改变背包状态...");
            InventoryManager.Instance.AddItem(m_StackableItemIds[0], 99);
        }

        // 读取之前保存的数据
        DebugEx.Log(nameof(InventoryTester), "加载之前保存的背包数据...");
        InventoryManager.Instance.LoadInventory(saveData);

        DebugEx.Success(nameof(InventoryTester), "存档与读档测试完成");
    }
    ...
    public void TestEdgeCases()
    {
        ...
        // 测试1: 添加不存在的物品
        DebugEx.Log(nameof(InventoryTester), "测试添加不存在的物品 ID:99999");
        bool result1 = InventoryManager.Instance.AddItem(99999, 1);
        DebugEx.Log(nameof(InventoryTester), $"结果: {(result1 ? "成功" : "失败（预期）")}");

        // 测试2: 移除不存在的物品
        DebugEx.Log(nameof(InventoryTester), "测试移除不存在的物品 ID:99999");
        bool result2 = InventoryManager.Instance.RemoveItem(99999, 1);
        DebugEx.Log(nameof(InventoryTester), $"结果: {(result2 ? "成功" : "失败（预期）")}");
        ...
        if (m_StackableItemIds.Count > 0)
        {
            ...
            DebugEx.Log(nameof(InventoryTester), $"结果: {(result3 ? "成功" : "失败（预期）")}");
        }

        // 测试4: 使用空格子
        DebugEx.Log(nameof(InventoryTester), "测试使用空格子");
        var slots = InventoryManager.Instance.GetAllSlots();
        for (int i = 0; i < slots.Count; i++)
        {
            if (slots[i].IsEmpty)
        ...
        // 测试5: 使用越界索引
        DebugEx.Log(nameof(InventoryTester), "测试使用越界索引 -1 和 999");
        ...
        DebugEx.Success(nameof(InventoryTester), "边界情况测试完成");
    }
    ...
    public void PrintInventoryStatus()
    {
        ...
        DebugEx.Log(nameof(InventoryTester), $"背包容量: {usedCount}/{maxCount}");
        DebugEx.Log(nameof(InventoryTester), "物品列表:");
        ...
        foreach (var kvp in itemCounts)
        {
            ...
            DebugEx.Log(nameof(InventoryTester), $"  - {itemName} (ID:{kvp.Key}) x{kvp.Value}");
        }

        DebugEx.Success(nameof(InventoryTester), "背包状态打印完成");
    }
    ...
    {
        DebugEx.LogColor(nameof(InventoryTester), "========== 清空背包 ==========", DebugEx.Color.Red);
        ...
        DebugEx.Success(nameof(InventoryTester), "背包已清空");
    }

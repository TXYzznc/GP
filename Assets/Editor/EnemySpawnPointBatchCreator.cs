...
public class EnemySpawnPointBatchCreator : EditorWindow
{
    ...
    [MenuItem("Window/EnemySpawnPoint Batch Creator")]
    private static void ShowWindow()
    {
        GetWindow<EnemySpawnPointBatchCreator>("Spawn Creator");
    }

    private void OnGUI()
    {
        GUILayout.Label("敌人生成点批量创建器（全场景 NavMesh 智能生成）", EditorStyles.boldLabel);
    ...
    private void GenerateSpawnPoints()
    {
        ...
        EditorUtility.DisplayDialog("成功", $"已生成 {selectedPoints.Count} 个 SpawnPoint（设置数量: {m_Count}）", "确定");
        Debug.Log($"已批量生成 {selectedPoints.Count} 个 EnemySpawnPoint");
    }

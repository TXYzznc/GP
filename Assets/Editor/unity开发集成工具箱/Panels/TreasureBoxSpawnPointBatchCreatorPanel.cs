...
/// <summary>
/// 宝箱生成点批量创建面板 - 在工具箱中快速创建多个 TreasureBoxSpawnPoint
/// </summary>
[ToolHubItem("场景工具/宝箱生成点批量创建器", "智能批量生成 TreasureBoxSpawnPoint，支持 NavMesh 约束和密度控制", 26)]
public class TreasureBoxSpawnPointBatchCreatorPanel : IToolHubPanel
{
    private int m_Count = 3;
    private float m_MinDistance = 8f;
    private float m_Radius = 1f;
    private float m_NavSampleRadius = 2f;
    private int m_MaxAttempts = 100000;
    private float m_MinDistanceFromEdge = 1f;
    ...
    public void OnGUI()
    {
        ...
        m_Count = EditorGUILayout.IntSlider("数量", m_Count, 1, 50);
    ...
    {
        return "批量生成 TreasureBoxSpawnPoint，支持 NavMesh 约束和密度控制。" +
               "可快速为场景布置多个宝箱生成点，参数独立于敌人生成。";
    }
    ...
    private void GenerateSpawnPoints()
    {
        ...
        var parentGO = new GameObject("TreasureBoxSpawnPoints");
        Undo.RegisterCreatedObjectUndo(parentGO, "Batch Create TreasureBox SpawnPoints");
        ...
        EditorUtility.DisplayDialog("成功", $"已生成 {selectedPoints.Count} 个宝箱 SpawnPoint", "确定");
        Debug.Log($"已批量生成 {selectedPoints.Count} 个 TreasureBoxSpawnPoint（设置数量: {m_Count}）");
    }
    ...
    {
        var go = new GameObject($"TreasureBoxSpawnPoint_{parent.childCount}");
        ...
        if (typeField != null)
            typeField.SetValue(spawnPoint, SpawnPointType.TreasureBox);
        ...
        Undo.RegisterCreatedObjectUndo(go, "Create TreasureBox SpawnPoint");
    }
}

...
public class ToolHubWindow : EditorWindow
{
    ...
    private void OnEnable()
    {
        ...
        // 恢复当前面板
        if (
            settings.tools.Count > 0
            && settings.selectedIndex >= 0
            && settings.selectedIndex < settings.tools.Count
        )
        {
    ...
    private void InitStyles()
    {
        ...
        tabStyle = new GUIStyle(EditorStyles.toolbarButton) { fixedHeight = 28, fontSize = 12 };
    ...
    private void DrawTopBar()
    {
        using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
        {
            ...
            using (new EditorGUI.DisabledScope(settings.locked))
            {
                ...
                if (
                    settings.tools.Count > 0
                    && GUILayout.Button("➖ 移除", EditorStyles.toolbarButton, GUILayout.Width(60))
                )
                    RemoveCurrentTool();
            }
    ...
    private void DrawTabs()
    {
        ...
        scrollPosition = EditorGUILayout.BeginScrollView(
            ...
            false, // 不显示垂直滚动条
            false, // 水平滚动条按需显示
            GUI.skin.horizontalScrollbar, // 水平滚动条样式
            GUIStyle.none, // 隐藏垂直滚动条
            EditorStyles.toolbar, // 背景用 toolbar 样式
            GUILayout.Height(34) // Tab 行高度（不含滚动条）
        );
        ...
        using (new EditorGUILayout.HorizontalScope())
        {
            for (int i = 0; i < settings.tools.Count; i++)
            {
                ...
                // 按文字内容计算精确宽度，加 12px 内边距
                float btnWidth = style.CalcSize(new GUIContent(displayName)).x + 12f;

                if (GUILayout.Button(displayName, style, GUILayout.Width(btnWidth)))
                {
            ...
            foreach (var t in types)
            {
                ...
                else
                {
                    menu.AddItem(
                        ...
                        false,
                        () => AddTool(t, typeName, description)
                    );
                }
            }
        }
    ...
    private void RemoveCurrentTool()
    {
        ...
        settings.selectedIndex = Mathf.Clamp(
            settings.selectedIndex,
            0,
            Mathf.Max(0, settings.tools.Count - 1)
        );
    ...
    private IToolHubPanel GetOrCreatePanel(string typeName)
    {
        ...
        if (type == null)
            return null;
        if (!typeof(IToolHubPanel).IsAssignableFrom(type))
            return null;

...
public class OutlineRenderFeature : ScriptableRendererFeature
{
    ...
    {
        outlineData = null;
        if (_outlineDatas.Count <= 0 || renderers.Count <= 0)
            return false;

        int targetId = renderers[0].transform.GetInstanceID();
        foreach (var data in _outlineDatas)
        {
            if (targetId == data.Id)
            {
                outlineData = data;
    ...
    public void DrawOrUpdateOutlines(List<Renderer> renderers, Color color, float outlineSize)
    {
        ...
        var id = renderers[0].transform.GetInstanceID();
        var data = new OutlineData(id, renderers, color, outlineSize);
        _outlineDatas.Add(data);
        _isReady = true;
    }

    public void DrawOutlines(List<Renderer> renderers)
    {
        DrawOutlines(renderers, DefaultColor, DefaultBlurSize);
    }

    public void DrawOutlines(List<Renderer> renderers, Color color)
    {
        DrawOutlines(renderers, color, DefaultBlurSize);
    }

    public void DrawOutlines(List<Renderer> renderers, Color color, float outlineSize)
    {
        if (renderers.Count <= 0)
            return;

        if (CheckId(renderers, out _))
            return;

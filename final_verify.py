...
expected_data = {
    ...
    201: {'name': '暗影咒体', 'params': '0.1,0.8,0.3'},
    202: {'name': '生命虹吸', 'params': '0,0.20,0.15'},
}
...
for row_idx in range(5, ws.max_row + 1):
    skill_id_cell = ws.cell(row_idx, 1)
    ...
    if skill_id in expected_data:
        ...
        if name_match and params_match:
            print(f'✓ ID {skill_id}: {actual_name} | Params: {actual_params}')
        else:
            print(f'❌ ID {skill_id}:')
            if not name_match:

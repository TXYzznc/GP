...
def draw_radar(profession_function, data, output_filename, index):
    """绘制单个职业-功能的雷达图"""
    ...
    # 添加说明文字
    plt.figtext(0.5, 0.02, 'Description: Values represent relative weights of attributes in the role-function',
    ...
    print("[{:02d}] Saved: {}".format(index, output_filename))
    plt.close()

# 生成所有 14 张图（15个组合中有14个有数据）
print("Start generating radar charts (5 professions x 3 functions)...")
print("-" * 60)

# 职业和功能的有序列表
professions = ['战士', '刺客', '法师', '坦克', '射手']
functions = ['输出', '辅助', '控制']

# 职业显示名称
prof_display = {'战士': 'Warrior', '刺客': 'Assassin', '法师': 'Mage',
                '坦克': 'Tank', '射手': 'Archer'}
func_display = {'输出': 'Output', '辅助': 'Support', '控制': 'Control'}
...
for profession in professions:
    for function in functions:
        key = f'{profession}_{function}'

        # 检查数据是否存在
        if key not in role_function_data:
            print("[{:02d}] SKIP: {} + {} (no data)".format(index, profession, function))
            index += 1
            continue

        data = role_function_data[key]
        filename = f'{index:02d}_{prof_display[profession]}_{func_display[function]}.png'

        draw_radar(key, data, filename, index)
        total_count += 1
        index += 1

print("-" * 60)
print(f"Successfully generated {total_count} charts!")

...
**SneakAttack处理**：
```
显示SneakDebuffSelectionUI
    ├─ 获取3个可选Debuff
    ├─ 生成3个BuffChooseItem选项
    └─ 等待玩家点击选择
...
```
随机选择先手Buff
    ├─ 显示InitiativeBuffNotificationUI
    ├─ 应用Buff到相应方
    └─ 自动关闭UI(3秒)
        └─ 进入战斗
```
...
```
脱战成功
├─ 增加污染值 (CorruptionCost)
├─ 返回探索状态
└─ 不进入冷却

脱战失败
├─ 降低召唤师生命 (HealthLossPenalty%)
├─ 进入冷却状态 (CooldownTurns回合)
└─ 继续战斗
```
...
CombatOpportunityDetector.Update()
    ...
    │   ├─ CheckSneakAttackOpportunity()
    │   │   └─ ShowOpportunityUI(SneakAttack)
    │   ├─ CheckEncounterOpportunity()
    │   │   └─ ShowOpportunityUI(Encounter)
    │   └─ 无机会
    │       └─ HideCombatInteract()
    │
...
CombatPreparationState.OnEnter()
    └─ ApplyInitiativeEffects()
        ├─ SneakAttack
        │   ├─ GetSneakDebuffPool() → [3001, 3002, 3003]
        │   ├─ OpenSneakDebuffSelectionUI(debuffIds)
        │   │   ├─ 生成3个BuffChooseItem
        │   │   └─ 等待选择
        │   └─ OnDebuffSelected(buffId)
        │       └─ ApplyBuffToEnemy(buffId)
        │
        ├─ Encounter
        │   ├─ GetRandomInitiativeBuff() → buffId
        │   ├─ ApplyPlayerInitiativeBuff(buffId)
        │   └─ ShowInitiativeBuffNotification("玩家", buffId)
        │
        └─ EnemyInitiated
            ├─ ApplyEnemyInitiativeBuff(buffId)
            └─ ShowInitiativeBuffNotification("敌人", buffId)
```
...
│   │   ├── Enemy/
│   │   │   ├── Core/
│   │   │   │   ├── EnemyEntity.cs ✏️ (已集成VisionDetector)
│   │   │   │   └── EnemyEntityAI.cs ✏️ (已调用UpdateDetection)
│   │   │   └── Detection/
...
│
├── UI/
│   ├── SneakDebuffSelectionUI.cs ✨ (偷袭选择)
│   ├── SneakDebuffSelectionUI.Variables.cs (自动生成)
│   │
│   ├── InitiativeBuffNotificationUI.cs ✨ (先手通知)
│   ├── InitiativeBuffNotificationUI.Variables.cs (自动生成)
│   │
│   ├── EscapeResultUI.cs ✨ (脱战结果)
│   ├── EscapeResultUI.Variables.cs (自动生成)
│   │
│   └── Item/
│       ├── EnemyMask.cs ✨ (敌人指示器)
│       ├── EnemyMask.Variables.cs (自动生成)
│       │
│       ├── BuffChooseItem.cs ✨ (Buff选项)
│       └── BuffChooseItem.Variables.cs (自动生成)
│
└── System/
...
### EscapeRuleTable（Excel）- 脱战规则（新建）

| 字段 | 类型 | 默认值 | 说明 |
|-----|-----|-------|------|
| Id | int | - | 规则ID |
| EnemyType | int | - | 敌人类型(0=普通, 1=精英, 2=Boss) |
| BaseSuccessRate | float | 0.6 | 基础成功率 |
| TimeBonus | float | 0.05 | 每回合增加 |
| MaxSuccessRate | float | 0.9 | 上限成功率 |
| CorruptionCost | int | 10 | 污染值代价 |
| HealthLossPenalty | float | 0.2 | 生命损失比例 |
| CooldownTurns | int | 2 | 冷却回合数 |
...
### 检测频率优化
```
VisionConeDetector.UpdateDetection()
└─ 每0.15秒调用一次（不是每帧）

CombatOpportunityDetector.DetectCombatOpportunities()
└─ 每0.1秒检测一次

EnemyAlertUIManager 距离排序
└─ 每0.5秒重新排序一次
```
...
2. **完整战斗触发流程**
   - 玩家靠近 → 检测条件 → 显示交互UI → 按空格触发 → 进入战斗

3. **偷袭流程**
   - 潜入敌人背后 → 显示偷袭UI → 按空格 → 选择Debuff → 进入战斗

4. **脱战流程**
   - 战斗中 → 按脱战按钮 → 计算成功率 → 成功/失败处理 → 返回探索/继续战斗
...
### 当前限制
1. **单一玩家检测**：同时只检测一个玩家
2. **固定参数**：所有参数通过数据表配置，不支持运行时动态调整
3. **无视线遮挡**：不考虑遮挡物，仅基于距离和角度
4. **无记忆系统**：敌人丢失玩家后立即忘记

### 可能的改进
1. **视线遮挡检测**：添加射线检测(Raycast)验证视线
2. **记忆系统**：敌人丢失玩家后保留一段时间的位置信息
3. **听觉系统**：根据玩家移动速度和声音强度调整检测
4. **小队协作**：敌人之间可广播玩家位置
5. **昼夜系统**：根据时间调整视野参数
...
- **包含内容**：
  - AI战斗系统总体设计 (原375行)
  - AI战斗系统架构文档 (原541行) - 作为基础框架
  - AI战斗系统实现清单 (关键信息已合并)
- **维护人**：AI Assistant
- **相关文档**：战斗与探索系统优化/ 文件夹内的相关文档
